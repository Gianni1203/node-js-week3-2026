...
let nextId;
if(members.length > 0){
    nextId = Math.max(...members.map(member => member.id)) + 1;
}else{
    nextId = 1;
}
...
function filterByQuery(list, query) {
    if(query && query.level){
        return list.filter(member => member.level === query.level);
    }
    return list;
}
...
router.put('/:id', (req, res) => {
    const memberId = Number(req.params.id); // 將網址id 轉成數字型別
    const index = members.findIndex(m => m.id === memberId); // 找出該會員的索引位置
    if(index === -1){
        return res.status(404).json({error: '會員不存在'});
    }
    // 將舊會員資料展開，並將前端req.body 更新後的會員資料展開，接著蓋掉舊的會員資料
    const updateMember = {
        ...members[index],
        ...req.body
    }
    members[index] = updateMember; // 將合併後的最新會員資料，存回原本的位置中
